...
from classroom.models import StudentsInClass, StudentMarks, ClassAssignment, Subject_master, SubmitAssignment, Student, Teacher
...
def dashboard(request):
    ...
    teachermsg = get_object_or_404(models.Teacher, pk=request.user.id)
    teacher = Teacher.objects.filter(user=request.user)
    return render(request, 'classroom/dashboard.html', {'teacher': teacher, 'teachermsg': teachermsg})


def studentclass(request):
    if request.user.is_authenticated:
        student = Student.objects.filter(user=request.user)
        std_notice = get_object_or_404(models.Student, pk=request.user.id)

        std = request.user.Student
        assignment = SubmitAssignment.objects.filter(student=std)
        assignment_list = [x.submitted_assignment for x in assignment]

        context = {
            'student': student,
            'std_notice': std_notice,
            'std': std,
    ...
    if request.method == "POST":
        ...
        if user:
            if user.is_active:
                login(request, user)
                if user.is_student:
                    return redirect('/classroom/studentclass/')
                else:
...
def TeacherUpdateView(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            profile_updated = True
            messages.success(request, 'Profile updated Successfully! ')
            return redirect(f"/classroom/teacher/{pk}")
    else:
...
def add_marks(request):
    if request.method == 'POST':
        student = request.POST.get('studentobj')
        ass_name = request.POST.get('assignment_name')
        obtainmarks = request.POST.get('obtainmarks')
        maxmarks = request.POST.get('maxmarks')
        returnid = request.POST.get('returnid')
        submisstionid = request.POST.get('submisstionid')
        std = get_object_or_404(Student, email=student)

        marks = StudentMarks.objects.create(
            teacher=request.user.Teacher,
            student=std,
            submission_id=submisstionid,
            subject_name=ass_name,
            marks_obtained=obtainmarks,
            maximum_marks=maxmarks
        )
        marks.save()

        marked = SubmitAssignment.objects.get(id=submisstionid)
        marked.is_marked = True
        marked.temp_marks = obtainmarks
        marked.save()

        messages.success(request, 'Marks Given !')

        return redirect(f"/classroom/assignment_summary/{returnid}/")
...
def add_notice(request):
    ...
    if request.method == "POST":
        ...
        if notice.is_valid():
            ...
            messages.success(request, "Your Notice is added successfully.")
            # notice_sent = True
    else:
        notice = NoticeForm()
    return render(request, 'classroom/write_notice.html', {'notice': notice})
...
class add_student(LoginRequiredMixin, generic.RedirectView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            messages.success(
                self.request, '{} successfully added!'.format(student.name))
...
def upload_assignment(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, "Assignment is uploaded successfully.")
            # assignment_uploaded = True
    else:
        form = AssignmentForm()
    return render(request, 'classroom/upload_assignment.html', {'form': form})
...
def class_assignment(request):
    ...
    sem = student.sem
    std_course = student.std_course
    subject = Subject_master.objects.filter(course=std_course, semester=sem)
    assignment = SubmitAssignment.objects.filter(student=student)
    assignment_list = [x.submitted_assignment for x in assignment]

    # classassignment = ClassAssignment.objects.all()
    # print(subject)
    # print(classassignment.ass_sub_id)
    # subjectdic = {}
    # for i in subject:
    #     if i in assignment.ass_sub
    #     ass_count = SubmitAssignment.objects.filter(student = student, ass_sub = subject )
...
# teacher_ass = SubmitAssignment.objects.filter(teacher = request.user.Teacher)


@login_required
def submit_list(request):
    # teacher = request.user.Teacher
    assignment = ClassAssignment.objects.filter(teacher=request.user.Teacher)
    return render(request, 'classroom/submit_list.html', {'assignment': assignment})


def assignment_summary(request, ass_id):
    ass_details = ClassAssignment.objects.get(id=ass_id)
    submited_ass = SubmitAssignment.objects.filter(
        teacher=request.user.Teacher, submitted_assignment=ass_id)

    return render(request, 'classroom/assignment_summary.html', {"ass_details": ass_details, "submited_ass": submited_ass})
